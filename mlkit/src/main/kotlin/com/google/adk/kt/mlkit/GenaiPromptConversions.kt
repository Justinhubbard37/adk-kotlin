...
internal object GenaiPromptConversions {
  ...
  private fun Content.toMlKitContent(includeRoleMarkers: Boolean): MlKitContent? {
    ...
    for (part in parts) {
      // Dropped rather than replayed because ML Kit cannot take one back: its request `Part` is a
      // sealed class with no thought flag, so a thought sent again would read to the model as
      // something it had said out loud, and would spend the device's small context saying it.
  ...
  internal fun GenerateContentResponse.toLlmResponse(includeThoughts: Boolean): LlmResponse {
    ...
    val thoughtText =
      selectThoughtText(thoughtProcess.map { it.text }, candidates.size, includeThoughts)
  ...
  /**
   * The thought belonging to the candidate this response surfaces, or `null` when there is none,
   * the caller did not ask for one, or it cannot be told apart from another candidate's.
   *
   * AI Core emits one thought per candidate, but ML Kit sorts and dedupes the candidates by score
   * while leaving the thoughts in their original order, and exposes neither the score nor the
   * original index. So with more than one thought there is no way to tell which belongs to the
   * candidate we surface, and attaching the wrong reasoning to an answer is worse than attaching
   * none.
   *
   ...
   */
  internal fun selectThoughtText(
    thoughtTexts: List<String>,
    candidateCount: Int,
    includeThoughts: Boolean,
  ): String? {
    if (!includeThoughts) return null
    if (candidateCount > 1 || thoughtTexts.size > 1) {
      logger.warn {
        "Several candidates carry a thought; ML Kit does not say which belongs to the one surfaced, so no thought is returned."
      }
      return null
    }
    return thoughtTexts.firstOrNull()?.let(::withoutChannelHeader)?.takeIf { it.isNotEmpty() }
  }

  /**
   * Drops the `thought` line AI Core leaves at the top of its reasoning.
   *
   * Its control tokens wrap a channel as `<start>name\ncontent<end>` and the legacy extraction
   * keeps the whole span, header included. Streaming delivers the thought in arbitrary pieces, so
   * this only removes an exact prefix and never trims: a piece that splits the header is left alone
   * rather than mangled, and no space the aggregator needs is eaten.
   */
  private fun withoutChannelHeader(thought: String): String = thought.removePrefix("thought\n")
