...
class GenaiPromptConversionsTest {
  ...
  fun selectThoughtText_thoughtsDeclined_returnsNull() {
    assertThat(
        selectThoughtText(listOf("counting things"), candidateCount = 1, includeThoughts = false)
      )
      .isNull()
  }
  ...
  fun selectThoughtText_thoughtsRequested_returnsTheThought() {
    assertThat(
        selectThoughtText(listOf("counting things"), candidateCount = 1, includeThoughts = true)
      )
      .isEqualTo("counting things")
  }

  /** Attaching another candidate's reasoning to this answer would be worse than attaching none. */
  @Test
  fun selectThoughtText_severalThoughts_returnsNull() {
    val thoughts = listOf("first thought", "second thought")

    assertThat(selectThoughtText(thoughts, candidateCount = 1, includeThoughts = true)).isNull()
  }

  /** AI Core leaves its channel name on the first line; it is not part of the reasoning. */
  @Test
  fun selectThoughtText_channelHeader_isStripped() {
    val thought = "thought\nLet B be the cost of the bat."

    assertThat(selectThoughtText(listOf(thought), candidateCount = 1, includeThoughts = true))
      .isEqualTo("Let B be the cost of the bat.")
  }

  /** Reasoning that merely opens with the word is left alone. */
  @Test
  fun selectThoughtText_thoughtWordInProse_isKept() {
    val thought = "thought about it for a while.\nThen decided."

    assertThat(selectThoughtText(listOf(thought), candidateCount = 1, includeThoughts = true))
      .isEqualTo(thought)
  }

  /** One thought but several candidates is the same unknowable pairing, so nothing is surfaced. */
  @Test
  fun selectThoughtText_oneThoughtSeveralCandidates_returnsNull() {
    val thoughts = listOf("the reasoning")

    assertThat(selectThoughtText(thoughts, candidateCount = 3, includeThoughts = true)).isNull()
  }

  /** An empty thought is nothing to show. */
  @Test
  fun selectThoughtText_emptyThought_returnsNull() {
    assertThat(selectThoughtText(listOf(""), candidateCount = 1, includeThoughts = true)).isNull()
  }

  @Test
  fun selectThoughtText_noThoughts_returnsNull() {
    assertThat(selectThoughtText(emptyList(), candidateCount = 1, includeThoughts = true)).isNull()
  }
