 ...
 *   marked as thoughts, shown here as muted text. Switching it off drops the config again, so the
 *   model stops reasoning too. It needs a Gemini Nano that supports thinking mode.
 *
...
class MlKitChatActivity : ScopedExampleActivity() {
  ...
  override fun onDestroy() {
    // Release only once the scope has finished, because cancelling it does not wait and a turn may
    // still be inside ML Kit on another thread. The fields are read in the handler rather than
    // captured, so a model a rebuild assigns after this point is still closed.
    scope.coroutineContext.job.invokeOnCompletion {
      // Failures are swallowed: an uncaught one here would take the process down, and the screen
      // this belonged to is already gone.
      runCatching { runner?.close() }
      runCatching { generativeModel?.close() }
    }
    // super cancels the scope, which is what fires the handler above.
    super.onDestroy()
  }
